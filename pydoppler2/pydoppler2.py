...
class spruit:
    ...
    def __init__(self,force_install = False):
        ...
        self.plot=False                       # Plot in Python window
    ...
    def Foldspec(self):
        ...
        *** Remember to prepare the keywords before running ***
        ...
        try:
            ...
            print('Phase file - {} - is not accessible. Check "base_dir" and "list"'.format(self.list))
        inputs = np.loadtxt(self.list,dtype={'names': ('files', 'phase'),'formats': ('S14', 'f4')})
        ...
        for z,i in enumerate(inputs):
            ...
            self.signals.progress.emit(int(self.progress*75/len(inputs)))
    ...
    def Dopin(self,poly_degree=2, continnum_band=False,
        ...
        spec_avg=avgspec/len(self.pha)                #RETURN spec_avg 
...
#        plt.draw()
        ...
        y_lim=[avgspec[lop].min()/len(self.pha)*0.8,avgspec.max()/len(self.pha)*1.1]  #RETURN y_lim
        z = np.polyfit(self.wave[0][lop], yor, poly_degree)
        pz = np.poly1d(z)
        linfit = pz(self.wave[0])       #RETURN  linfit
        linfit2=pz(self.wave[0]) 
        plt.plot(self.wave[0],linfit,'r',label='Cont Fit')
        lg = plt.legend(fontsize=14)
        plt.xlim(xor[0]-10,xor[3]+10)
        x_lim=[xor[0]-10,xor[3]+10]         #RETURN x_lim
        plt.xlabel(r'Wavelength / $\AA$')
        plt.ylabel('Input flux')

#########################CHECAR AQUI PARA VER COMO CAMBIA A ESPACIO DE VELOCIDAD####################################
        ax=fig.add_subplot(212)
        vell=((self.wave[0]/self.lam0)**2-1)*cl/(1+(self.wave[0]/self.lam0)**2)   #RETURN vell
        ...
        y_lim2=[-0.05*np.max(avgspec[qq]/len(self.pha)-linfit[qq] -1.0),     #RETURN y_lim2
                np.max(avgspec[qq]/len(self.pha)-linfit[qq] -1.0)*1.1]
...
#        f.write(str(len(self.flux))+" "+str(self.nbins)+" "+str(self.lam0)+'\n')
        ...
        return spec_avg,xor,y_lim,linfit2,x_lim,vell,y_lim2,trail,extent
    ...
    def Dopmap(self,dopout = 'dop.out',cmaps = cm.Greys_r,
        ...
        if negative:
            if remove_mean:
                ...
                new_data=(data - meano)/(data - meano)[qq].max()
...
#        plt.show()
        ...
        return new_data,vpmin,vpmax


    def Reco(self, cmaps=plt.cm.binary,limits_spec=None, limits=None, colorbar=True):
        """
        ...
        ax1.set_xlabel('Velocity / km s$^{-1}$',fontsize=16)
        ax1.set_xticks([-1000,-500,-0,500,1000])
        ax1.set_xlim(-1200,1200)
        ax1.set_ylabel('Orbital Phase',fontsize=16)
        ...
        return dmr,dm,trail_dmr,trail_dm,x1_lim,x2_lim,phase[0],phase[-1]
...
def stream(q,k1,porb,m1,inc,colors='k',both_lobes=False,title=True,label=None):
    ...
    #print vx[ss],vy[ss]
    plt.plot(vx[:],vy[:],color=colors,marker='')  #RETURN vx vy
    
    plt.plot(vkx[:],vky[:],color=colors,marker='') #RETURN vkx, vky
    plt.plot(yl[int(yl.size/4):3*int(yl.size/4)],xl[int(xl.size/4):3*int(xl.size/4)],color=colors)
    ylx=yl[int(yl.size/4):3*int(yl.size/4)]    #RETURN ylx
    xly=xl[int(xl.size/4):3*int(xl.size/4)]    #RETURN xly
    
    if title: plt.title(r'$i$='+str(inc/np.pi*180.)[:2]+', M$_w$='+str(m1)+' M$_{\odot}$, $q$='+str(q)[:]+', P$_{orb}$='+str(porb/3600.)[:4]+' hr',fontsize=16)
    ## 0,0 systemic velocity, km/s
    vy1 = cm * vs * si
    plt.plot(0.,0.,'x',ms =9 ,c =colors,alpha=0.8)
    ## 0,-K1 systemic velocity, km/s
    plt.plot(0.,-vy1,'+',ms = 10,c =colors,alpha=0.7)     #RETURN vy1
    plt.plot(0.,(1.0-cm)*vs*si,'+',ms = 10,c =colors,alpha=0.7)
    vy2=(1.0-cm)*vs*si                                         #RETURN vy2
    conca1=np.concatenate((yl[3*int(xl.size/4):],yl[:int(yl.size/4)]),axis=0)   #RETURN conca1
    conca2=np.concatenate((xl[3*int(xl.size/4):],xl[:int(yl.size/4)]),axis=0)   #RETURN conca2
    conca3=yl[int(yl.size/4):3*int(yl.size/4)]
    conca4=xl[int(xl.size/4):3*int(xl.size/4)]  #RETURN conca3
    if both_lobes:
    ...
    plt.tight_layout()
    return vx,vy,vkx,vky,ylx,xly,vy1,vy2,conca1,conca2,conca3,conca4
